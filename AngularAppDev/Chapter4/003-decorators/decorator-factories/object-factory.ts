...
export class ObjectFactory<T> {
    ...
    createObject(json: string | object): T {
        ...
        // 如果目标属性被必需属性装饰器（工厂）Required装饰了，并且目标属性未获得到值
        // 那么抛出记录在目标属性的元数据customer:required上的错误提示
        let requiredProperties: string[] = Reflect.getMetadata('custom:requiredProperties', this._constructor.prototype) as string[] || [];
        for (let propertyName of requiredProperties) {
            if (targetObject[propertyName] == undefined || targetObject[propertyName] == null 
                || targetObject[propertyName].toString().trim() == '') {
                // 获取元数据：错误提示
                let requiredErrorMessage = Reflect.getMetadata('custom:required', this._constructor.prototype, propertyName);
                throw requiredErrorMessage;
            }
        }
